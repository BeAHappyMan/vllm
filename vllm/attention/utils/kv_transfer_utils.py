...
def maybe_transfer_kv_layer(func: Callable) -> Callable:
    ...
    def wrapper(*args, **kwargs):
        ...
        kv_load_start_time = time.time_ns()
        logger.info(f"KV cache transfer load start - layer: {layer_name}, timestamp: {kv_load_start_time}")
        connector.wait_for_layer_load(layer_name)
        kv_load_end_time = time.time_ns()
        logger.info(f"KV cache transfer load end - layer: {layer_name}, timestamp: {kv_load_end_time}, duration: {kv_load_end_time - kv_load_start_time}")

        # Execute the function
        result = func(*args, **kwargs)

        # Save KV cache layer on exit
        kv_save_start_time = time.time_ns()
        # logger.info(f"KV cache transfer save start - layer: {layer_name}, timestamp: {kv_save_start_time}")
        connector.save_kv_layer(layer_name, kv_cache, attn_metadata)
        kv_save_end_time = time.time_ns()
        logger.info(f"KV cache transfer save end - layer: {layer_name}, timestamp: {kv_save_end_time}, duration: {kv_save_end_time - kv_save_start_time}")
